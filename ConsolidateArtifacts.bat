...
set lib_path=%lib_path%\qtbase\bin\%platform%-%msvcVersion%

if exist "%destination%" rmdir "%destination%" /s/q

echo "Packaging library"
xcopy "%lib_path%\include"  "%destination%\include" /y/s/q/i
if !errorlevel! neq 0 exit /b !errorlevel!
xcopy "%lib_path%\lib"  "%destination%\lib" /y/s/q/i
if !errorlevel! neq 0 exit /b !errorlevel!
xcopy "%lib_path%\bin"  "%destination%\bin" /y/s/q/i
if !errorlevel! neq 0 exit /b !errorlevel!
xcopy "%lib_path%\plugins"  "%destination%\plugins" /y/s/q/i
if !errorlevel! neq 0 exit /b !errorlevel!
